...
public class Common {
	...
	public static void noticeMail(String content) throws UnsupportedEncodingException, MessagingException {
		...
		if (Prop.get("notice.mail.sender.type").equalsIgnoreCase("qq")) {
			...
			// MailSender.sendEmailFastByAuth(senderAccount, senderPasswd, recverAccounts,
			// content, Prop.get("notice.mail.sender.qq.smtpport"));
			AsyncQQMailSender.sendMail(senderAccount, senderPasswd, recverAccounts, content,
					Prop.get("notice.mail.sender.qq.smtpport"));
	 ...
	 */
	public static void captureImageAndSave(RobotManager robotMngr, int x, int y, int width, int height)
			throws IOException {
		...
		BufferedImageUtils.captureImageAndSave(robotMngr, x, y, width, height, dirpath,
				DateUtils.timeStrWithMillisWithoutPunctuation());
	}
	...
	public static void noticeSound() {
		...
		if (Prop.getBool("notice.sound.flag") && !isInNoticeSoundExcludetime()) {
			// WavPlayer.play(Prop.get("notice.sound.wavpath"),
			// Prop.getInt("notice.sound.playcount"));
	...
	public static void noticeAlertSound() {
		// WavPlayer.play(Prop.get("notice.exceptionsound.wavpath"),
		// Prop.getInt("notice.exceptionsound.playcount"));
		AsyncSoundMaker.makeSound(Prop.get("notice.exceptionsound.wavpath"),
				Prop.getInt("notice.exceptionsound.playcount"));
	...
	public static boolean comparePic(RobotManager robotMngr, String prefix) throws IOException {
		boolean flag = false;
		BufferedImage image = robotMngr.createScreenCapture(Prop.getInt(prefix + ".x"), Prop.getInt(prefix + ".y"),
				Prop.getInt(prefix + ".width"), Prop.getInt(prefix + ".height"));
	...
	public static void createBatDirectToFinishedWindows(File dstDir) throws IOException {
		...
		FileUtils.writeToFile(file.getCanonicalPath(), "start /max explorer \"" + dstDir.getCanonicalPath() + "\"",
				false, "GBK");
	}

	// 当前是否处于邮件通知的排除时段
	public static boolean isInNoticeMailExcludetime() {
		if (!Prop.getBool("notice.mail.excludetime.need"))
			return false;
	...
	public static boolean isInNoticeSoundExcludetime() {
		if (!Prop.getBool("notice.sound.excludetime.need"))
			return false;
	...
	public static void main(String[] args) throws UnsupportedEncodingException, MessagingException {
		/*
		 * noticeMail("test"); noticeSound(); AsnycHiddenFileDeleter.delete(new
		 * File(Prop.get("srcdirpath")));
		 */
		File dir = new File("D:\\_caj2pdf\\cajwait");
		for (File f : dir.listFiles()) {
			if (f.isDirectory()) {
				
				if(containsSpecCharacters(f.getAbsolutePath())){
					/*System.out.println(f.getAbsolutePath());
					 System.out.println(new File(f.getAbsolutePath()).exists()); */
					System.out.println(deleteSpecCharacters(f.getAbsolutePath()));
				}
